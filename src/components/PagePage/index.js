...
class Page extends Component {
  ...
  render() {
    ...
    console.log(components, "00000000");
    const componentIndex = this.state.componentIndex;
    const pageComponents = page.components.map((component, index) => {
      ...
      return (
        <div
          className="component"
          className={className}
          onTouchTap={self.showEditComponentDialog.bind(self, index, component)}
        ...
        >
          ...
          <div className="add-component-btn-wrap">
            <FlatButton
              label="添加"
              secondary={true}
              onTouchTap={this.addComponentToPage.bind(this, component)}
    ...
    return (
      <div className="PagePage">
        <Header />
        <LeftNav
          open={this.state.showComponents}
          width={this.state.deviceWidth}
        >
          <button
            onTouchTap={this.hideComponents}
        ...
        <SelectField
          floatingLabelText={
            this.state.deviceWidth + "*" + this.state.deviceHeight
          }
          hintText="选择设备"
          autoWidth={true}
          value={this.state.deviceWidth + "*" + this.state.deviceHeight}
        >
          <MenuItem
            value="320*480"
            primaryText="iPhone4"
            onClick={this.handleChangeDevice.bind(this, 320, 480)}
            className="device-radio"
          />
          <MenuItem
            value="320*568"
            primaryText="iPhone5"
            onClick={this.handleChangeDevice.bind(this, 320, 568)}
            className="device-radio"
          />
          <MenuItem
            value="375*627"
            primaryText="iPhone6"
            onClick={this.handleChangeDevice.bind(this, 375, 627)}
            className="device-radio"
          />
          <MenuItem
            value="414*736"
            primaryText="iPhone6S"
            onClick={this.handleChangeDevice.bind(this, 414, 736)}
            className="device-radio"
          />
        </SelectField>
        ...
        >
          ...
          >
            <iframe
              ...
              style={{
                width: this.state.deviceWidth - 20,
                height: this.state.deviceHeight - 20
              }}
              src={`/#/preview/${this.state.page._id}`}
              frameBorder="0"
            />
          </div>
        </Resizable>
        <LeftNav
          open={this.state.showComponentSettings}
          width={this.state.editorSliderValue * this.props.viewport.width}
          openRight={true}
        >
          <button
            onTouchTap={this.hideComponentSettings}
          ...
          <div
            id="editor-holder"
            style={{ height: this.props.viewport.height - 150 }}
          />
          <div
            className="right editor-slider-wrap"
            style={{ width: this.props.viewport.width * 0.2 }}
          >
            <Slider
              name="editorSlider"
              onChange={this.handleChangeEditorSlider}
              value={this.state.editorSliderValue}
              min={0.3}
              max={0.9}
              step={0.01}
            />
          ...
          <div className="component-action">
            <RaisedButton
              label="删除组件"
              primary={true}
              onTouchTap={this.deletePageComponent}
            />
          </div>
        </LeftNav>
        <div className="page-actions">
          <RaisedButton
            label="添加组件"
            primary={false}
            onTouchTap={this.toggleComponents}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
          <RaisedButton
            label="保存配置"
            secondary={true}
            onTouchTap={this.updatePage}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
          <RaisedButton
            label="设置页面信息"
            primary={false}
            onTouchTap={this.showEditPageInfoDialog}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
          <RaisedButton
            label="生成页面"
            primary={true}
            onTouchTap={this.generatePage}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
          <RaisedButton
            label="打包下载"
            secondary={false}
            linkButton={true}
            href={`/api/download/${this.state.page._id}`}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
          <RaisedButton
            label="发布上线"
            primary={true}
            onTouchTap={this.publishPage}
            disabled={this.state.pageGenerating || this.state.pagePublishing}
          />
